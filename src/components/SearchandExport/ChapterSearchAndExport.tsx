...
type ChapterSearchAndExportProps = {
  searchTerm: string;
  setSearchTerm: (term: string) => void;
  rowsPerPage: number;
  setRowsPerPage: (rows: number) => void;
  selectedChapter: number; // The selected section
  chapterName: string;     // The name of the section to use in the file name
  fullChapterData: VerseData[];  // The full section data to be exported (without pagination)
};
...
const SectionSearchAndExport = ({
  ...
  const exportToExcel = async () => {
    ...
    worksheet.columns = [
      { header: 'Chapter Number', key: 'Chapter_Number', width: 20 },
    ...
    fullChapterData.forEach((row) => {
      worksheet.addRow(row);
    });

    const buffer = await workbook.xlsx.writeBuffer();
    const fileName = `${chapterName}_section_${selectedChapter}.xlsx`;  // Use the section name and number in the file name
